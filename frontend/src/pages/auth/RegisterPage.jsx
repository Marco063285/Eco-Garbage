...
export default function RegisterPage() {
  ...
  const handleSubmit = async (e) => {
    ...
    if (form.phone && !isValidCmPhone(form.phone))
      return toast.error(isEn ? 'Invalid Cameroonian phone number' : 'Numéro de téléphone camerounais invalide')
    ...
    try {
      ...
      if (form.role === 'collector') {
        if (!idFront || !idBack || !selfiePhoto) {
          throw new Error(isEn ? 'Collectors must provide ID documents and a selfie.' : "Les collecteurs doivent fournir les pièces d'identité et un selfie.")
        }
        if (!form.national_id_number || idError) {
          throw new Error(isEn ? 'Valid national ID number is required.' : "Numéro de carte d'identité requis et valide.")
        }
          ...
          {registered ? (
            ...
            <div className="bg-white rounded-3xl shadow-green-lg p-8">
              ...
              <form onSubmit={handleSubmit} className="space-y-4">
                ...
                <div>
                  ...
                  <input type="tel" className={`input ${phoneError ? 'border-red-400 focus:ring-red-200' : ''}`}
                    ...
                    onChange={e => {
                      ...
                      set('phone', formatted)
                      setPhoneError(formatted.replace(/[\s]/g, '').length > 4 && !isValidCmPhone(formatted) ? (isEn ? 'Invalid number (e.g. +237 6 XX XX XX XX)' : 'Numéro invalide (ex: +237 6 XX XX XX XX)') : '')
                    }} />
                ...
                {form.role === 'collector' && (
                  <div>
                    <label className="label">{isEn ? 'National ID number' : "Numéro de carte d'identité nationale"} <span className="text-red-500">*</span></label>
                    <input type="text" className={`input ${idError ? 'border-red-400 focus:ring-red-200' : ''}`}
                      ...
                      onChange={e => {
                        ...
                        if (value && (value.length < 8 || value.length > 20 || !/^[A-Z0-9]+$/.test(value))) {
                          setIdError(isEn ? 'Invalid format (8–20 chars, letters and digits only)' : 'Format invalide (8-20 caractères, lettres et chiffres uniquement)')
                        } else {
                          setIdError('')
                        }
                      }} />
                    {idError && <p className="text-xs text-red-500 mt-1">{idError}</p>}
                    <p className="text-xs text-gray-400 mt-1">{isEn ? 'Enter your exact ID card or passport number.' : "Entrez le numéro exact de votre carte d'identité ou passeport"}</p>
                  </div>
                )}

                {form.role === 'collector' && (
                  <div className="space-y-4 rounded-2xl border border-gray-200 bg-gray-50 p-4">
                    <p className="text-sm font-semibold text-gray-800">{isEn ? 'Collector documents' : 'Documents collecteur'}</p>
                    <p className="text-xs text-gray-500">{isEn ? 'Upload both sides of your ID and a clear selfie.' : "Téléchargez les deux faces de votre pièce d'identité et un selfie clair."}</p>
                    <div>
                      <label className="label">{isEn ? 'ID card — Front' : "Carte d'identité — Recto"} <span className="text-red-500">*</span></label>
                      <input type="file" accept="image/*" className="input" onChange={e => setIdFront(e.target.files?.[0] || null)} />
                    </div>
                    <div>
                      <label className="label">{isEn ? 'ID card — Back' : "Carte d'identité — Verso"} <span className="text-red-500">*</span></label>
                      <input type="file" accept="image/*" className="input" onChange={e => setIdBack(e.target.files?.[0] || null)} />
                    </div>
                    <div>
                      <label className="label">{isEn ? 'Selfie (photo)' : 'Selfie (photo de présence)'} <span className="text-red-500">*</span></label>
                      <input type="file" accept="image/*" className="input" onChange={e => setSelfiePhoto(e.target.files?.[0] || null)} />
                    </div>
                    <div>
                      <label className="label">{isEn ? 'Face video (optional)' : 'Vidéo du visage (optionnel)'}</label>

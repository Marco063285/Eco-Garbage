...
export default function ResetPasswordPage() {
  ...
  const handleSubmit = async (e) => {
    e.preventDefault()
    if (!password || password.length < 8) return toast.error('Le mot de passe doit contenir au moins 8 caractères')
    ...
    try {
      ...
      if (err.response?.data?.message?.includes('expire')) {
        setStatus('error')
      }
      ...
      <div className="w-full max-w-md bg-white rounded-3xl shadow-green-lg p-10 text-center">
        ...
        {status === 'success' && (
          <>
            ...
            <h2 className="text-2xl font-display font-bold mb-2">Mot de passe réinitialisé !</h2>
        ...
        {status === 'error' && (
          <>
            ...
            <Link to="/forgot-password" className="btn-primary w-full justify-center py-3">Demander un nouveau lien</Link>
          </>
        )}

        {status === 'form' && (
          <>
            <h2 className="text-2xl font-display font-bold mb-2">Nouveau mot de passe</h2>
            ...
            <form onSubmit={handleSubmit} className="space-y-5 text-left">
              <div>
                ...
                <p className="text-xs text-gray-400 mt-1">Doit contenir une majuscule, une minuscule et un chiffre</p>
              </div>
              <div>
